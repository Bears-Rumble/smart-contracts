...
async function main() {
  ...
  const saleOneEnd = saleOneStart + 60 * 60 * 24 * 30; // 4 days
  ...
  const saleTwoEnd = saleTwoStart + 60 * 60 * 24 * 3; // 3 days
  ...
  const saleThreeStart = saleTwoEnd + 60 * 60 * 6; // 6 hours after SaleTwo ends
  const saleThreeEnd = saleThreeStart + 60 * 60 * 24 * 1; // 1 day
